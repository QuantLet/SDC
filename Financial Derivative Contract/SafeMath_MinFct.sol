...
//Mod SafeMath lib - includes a min function
library SafeMath {
  ...
  function div(uint256 a, uint256 b) internal pure returns (uint256) {
    // assert(b > 0); // Sol breaks dividing by 0
    uint256 c = a / b;
    // assert(a == b * c + a % b); // 100% true
    return c;
  }
...
//CRIX_Swap_Token; ERC20 compliant token representing payout of the swap SDC specified in the Factory SD
//Each Factory SC specifies one CRIX_Swap_Token and token address can contain many different swap SDC that are standardized at the Factory level
contract CRIX_Swap_Token {

  using SafeMath for uint256;

  /*Structs*/
  //Keeps track of balance amounts in balances array
  struct Balance {
    address owner;
    uint amount;
  }

  //Factory SC that token is standardized at
  address public master_contract;
  //Total supply of outstanding tokens in SD
  ...
  /*Functions*/
  //Constructor
  function CRIX_Swap_Token(address _factory) public {
    //Sets values for token name and token supply, as well as the master_contract, the swap.
    master_contract = _factory;
  }
  //Token Creator - This function is called by the factory SD and creates new tokens for the user
